...
function get_complete_intersection()
  p = 3
  R, vars = polynomial_ring(GF(p),5)
  two = random_homog_poly_mod(p,vars,2)
  three = random_homog_poly_mod(p,vars,3)

  (two,three)
end

function is_candidate_for_non_F_split_CI(f,g)
  #finbracket = MMPSingularities.inPowerOfVariableIdeal(3,3,f)
  #ginbrakcet = MMPSingularities.inPowerOfVariableIdeal(3,3,g)
  #f2inbracket = MMPSingularities.inPowerOfVariableIdeal(3,3,f^2)
  #g2inbracket = MMPSingularities.inPowerOfVariableIdeal(3,3,g^2)

  #inbracket = MMPSingularities.inPowerOfVariableIdeal(3,3,(f*g)^2)
  inbracket = inPowerOfVariableIdeal(3,3,(f*g)^2)

  inbracket
end

function find_candidates_for_non_F_split_CI(N,alert=50)
  k = 0
  while k < N 

    f, g = get_complete_intersection()
    cand = is_candidate_for_non_F_split_CI(f,g)
    if cand
    ...
    if k % alert == 0
      println("tried $alert examples")
    end
  end
end


function base_p_expansion(n,p)
    expansion = zeros(Int,0)
    q = n
    while 0 < q
        push!(expansion,q % p)
        q ÷= p
...
function add_base_p_with_carries(x,y,p)
    ...
    sum = zeros(Int,l+1)
    nCarries = 0
    for r in 1:length(short)
        s = short[r] + long[r] + sum[r]
        if p ≤ s
            s -= p
            nCarries += 1
            sum[r+1] += 1
    ...
    for r in length(short)+1:length(long)
        s = long[r] + sum[r]
        if p ≤ s
            s -= p
            nCarries += 1
    ...
    finalentry = findlast(!=(0),sum)

    (sum[1:finalentry],nCarries)
end

#    function test_add_base_p()
#    
#        x = base_p_expansion(16,2)
#        @Test add_base_p_with_carries(ans,ans,2) == ([0, 0, 0, 0, 0, 1], 1)
#        
#        one = base_p_expansion(1,2)
#        three = base_p_expansion(3,2)
#        @Test add_base_p_with_carries(one,three,2) == ([0, 0, 1], 2)
#    end
#    
#    function pascal_valuations(n,p)
#        result = zeros(Int,0)
#        for m in 1:n÷2
#            m_basep = base_p_expansion(m,p)
#            nminusm_basep = base_p_expansion(n-m,p)
#            (sum, carries) = add_base_p_with_carries(m_basep,nminusm_basep,p)
#            push!(result,carries)
#        end
#        result
#    end
...
function generic_power_formula(pow,d,n)
