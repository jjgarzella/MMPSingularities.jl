...
function run_experiment(;numVars = 4, prime = 5, howHigh = 9, time = Second(100))
    ...
    while (now() - startTime) < time
        ...
        try
	    	...
	    	# Because of the bug, we also need to check with a guaranteed working version
		        ...
		        println("Underestimated! $str")
                        open("experiments/CalabiYau/char5/underestimated.txt", "a") do file
		    ...
		    open("experiments/CalabiYau/char5/overestimated.txt", "a") do file
                        write(file, "Matrix method returned $height, safe method returned $height, polynomial: $poly\n")
                    end
                end
            end
            ...
            str = "$e $poly\n"
            open("experiments/CalabiYau/char5/erroredpolynomials.txt", "a") do file
                write(file, str)
            end
        ...
        if samples % 1000 == 0
            ...
            CSV.write("experiments/CalabiYau/char5/heights.csv", df, append=true)
            CSV.write("experiments/CalabiYau/char5/heightsbargraph.csv", dfheights, append=true)
            empty!(df)
            empty!(dfheights)
